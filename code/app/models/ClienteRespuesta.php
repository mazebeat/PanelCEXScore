...
class ClienteRespuesta extends \Eloquent
{
	...
	{
		if (!Auth::guest()) {
			$count = static::whereIdCliente(array(Auth::user()->id_cliente))->whereRaw('MONTH(ultima_respuesta) = MONTH(CURRENT_DATE) AND YEAR(ultima_respuesta) = YEAR(CURRENT_DATE)')->orderBy('id_cliente_respuesta', 'DESC')->count(array('id_cliente'));
			if ($count) {
				return true;
			}
		}
